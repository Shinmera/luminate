...
function buildMenu($menu){
    ...
    if($a->user->userID=='')
        ...
        $menu[]=array($a->user->displayname,$k->url("user","panel"),"float:right;",array(
    ...
    switch($_POST['action']){
        ...
        case 'Block':
            ...
            if($exists!=''){
                ...
                $l->triggerHook("FRIENDblock",$this,array($a->user->userID,$exists));
                $err[4]="User blocked.";
...
function displayUserPage($username){
    ...
    if($user==null){
        ...
        if($params[1]=='')$params[1]='Profile';
        $pages = array('Profile');
        $pages = $l->triggerHookSequentially('PROFILEnavbar',$this,$pages);
        ...
        switch($params[1]){
            case '':
            case 'Profile':$this->displayUserProfile($username,$user);break;
            case 'Add Friend':break;
            default:$l->triggerHook('PAGE'.$site,$this,array($user));break;
        }               
    }
