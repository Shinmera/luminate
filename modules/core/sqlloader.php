...
var $version=2.4;
var $short='c';
var $required=array('Parser'=>'core/parser.php');
...
function query($query,$data,$enparse=true){
    try{
    ...
    if($stmt->prepare($query)){
        $stmt=$this->bindVars($stmt,$data,$enparse);
...
function getData($query,$data=array(),$enparse=true){
    try{
    $stmt = $this->mysqli->stmt_init();
    if($stmt->prepare($query)){
        $stmt=$this->bindVars($stmt,$data,$enparse);
...
function bindVars($stmt,$params=array(),$enparse=true) {
    global $p;
    if (count($params)>0) {
        ...
        foreach($params as &$param) {        //for each element, determine type and add
            if(is_int($param)) {            $types .= 'i';
                                            ...
                                            if($enparse)$param=$p->enparse($param);
            } else {                        $types .= 'b';
            }
        }
